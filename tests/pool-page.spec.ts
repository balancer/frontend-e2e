...
test.describe('Pool page', () => {
  test('Add liquidity to Weighted pool', async ({ page, metamask, toast }) => {
    ...
    expect(await toast.get(/Add Liquidity pending/i)).toBeVisible();

    // Check the Add Liquidity confirmed toast shows up
    await toast.get(/Add Liquidity confirmed/i).waitFor({
      state: 'visible',
      // Increase timeout while waiting for the Add Liquidity to be confirmed
      timeout: 60000,
    });
    expect(await toast.get(/Add Liquidity confirmed/i)).toBeVisible();
  });
